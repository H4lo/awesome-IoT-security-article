...
- 摘要: Sophos published a security advisory for their Web Appliance product, highlighting a critical vulnerability that allows for command injection. However, the likelihood of mass exploitation is low due to certain caveats mentioned in the advisory. The vulnerability can be exploited through user-controlled input processed by PHP's escapeshellarg function. An RCE PoC was developed to demonstrate the exploitation. The command injection works by injecting a malicious command wrapped in single quotes, which can execute a reverse shell. Hunting for IOCs can be done by monitoring the /log/ui_access_log file.
